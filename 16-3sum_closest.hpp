...
public:
    int threeSumClosest(vector<int> &nums, int target)
    {
        ...
        if (smallest>=target) return smallest; //because any number after nums[2] is >= the first three thereby not changing or decreasing the sum.

        int max = nums[len-1] + nums[len-2] + nums[len-3];
        if (max <= target ) return max; // similarly any number before nums[len-3] is <= the last three therby not changing or increasing the sum.
